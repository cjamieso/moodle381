...
if (!empty($displaysection)) {
    ...
    $defaulttogglepersistence = clean_param(get_config('format_collblct', 'defaulttogglepersistence'), PARAM_INT);

    if ($defaulttogglepersistence == 1) {
        user_preference_allow_ajax_update('collblct_toggle_' . $course->id, PARAM_RAW);
        $userpreference = get_user_preferences('collblct_toggle_' . $course->id);
    } else {
        $userpreference = null;
    }

    $defaultuserpreference = clean_param(get_config('format_collblct', 'defaultuserpreference'), PARAM_INT);
    ...
    echo \format_collblct\toolbox::hex2rgba($tcsettings['togglebackgroundcolour'], $tcsettings['togglebackgroundopacity']);
    echo ';';
    echo '}';
    ...
    echo '.course-content ul.ctopics li.section .content.sectionhidden {';
    echo 'color: ';
    echo \format_collblct\toolbox::hex2rgba($tcsettings['toggleforegroundcolour'], $tcsettings['toggleforegroundopacity']);
    echo ';';
    echo 'text-align: ';
    ...
    echo 'color: ';
    echo \format_collblct\toolbox::hex2rgba($tcsettings['toggleforegroundhovercolour'], $tcsettings['toggleforegroundhoveropacity']);
    echo ';';
    echo '}';

    echo '.course-content ul.ctopics li.section .content div.toggle:hover {';
    echo 'background-color: ';
    echo \format_collblct\toolbox::hex2rgba($tcsettings['togglebackgroundhovercolour'], $tcsettings['togglebackgroundhoveropacity']);
    echo ';';
    echo '}';
    ...
    $tcborderradiustl = clean_param(get_config('format_collblct', 'defaulttoggleborderradiustl'), PARAM_TEXT);
    $tcborderradiustr = clean_param(get_config('format_collblct', 'defaulttoggleborderradiustr'), PARAM_TEXT);
    $tcborderradiusbr = clean_param(get_config('format_collblct', 'defaulttoggleborderradiusbr'), PARAM_TEXT);
    $tcborderradiusbl = clean_param(get_config('format_collblct', 'defaulttoggleborderradiusbl'), PARAM_TEXT);
