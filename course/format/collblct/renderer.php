...
// Include jquery here: must go before any page output.
global $PAGE;
...
class format_collblct_renderer extends format_section_renderer_base {
    ...
    public function __construct(moodle_page $page, $target) {
        ...
        $this->courserenderer = $this->page->get_renderer('format_collblct', 'course');
        ...
        $this->tctoggleiconsize = clean_param(get_config('format_collblct', 'defaulttoggleiconsize'), PARAM_TEXT);
    ...
    protected function section_header($section, $course, $onsectionpage, $sectionreturn = null) {
        ...
        $liattributes = array(
            ...
            'role' => 'region',
            'aria-label' => $this->courseformat->get_collblct_section_name($course, $section, false)
        );
        if (($this->formatresponsive) && ($this->tcsettings['layoutcolumnorientation'] == 2)) { // Horizontal column layout.
            $liattributes['style'] = 'width: ' . $this->tccolumnwidth . '%;';
        }
        ...
        if (($onsectionpage == false) && ($section->section != 0)) {
            ...
            if ($this->userisediting) {
                ...
                $o .= html_writer::tag('span',
                    $this->courseformat->get_collblct_section_name($course, $section, false),
            ...
            if ($this->userisediting && has_capability('moodle/course:update', $context)) {
                ...
                $o .= html_writer::link($url,
                    $this->output->pix_icon('t/edit', get_string('edit')),
                    array('title' => get_string('editsection', 'format_collblct'))
                );
            }
    ...
    protected function stealth_section_header($sectionno) {
        ...
        $liattributes = array(
            ...
            'role' => 'region',
            'aria-label' => $this->courseformat->get_collblct_section_name($course, $sectionno, false)
        );
        if (($this->formatresponsive) && ($this->tcsettings['layoutcolumnorientation'] == 2)) { // Horizontal column layout.
            $liattributes['style'] = 'width: ' . $this->tccolumnwidth . '%;';
        }
    ...
    protected function display_instructions() {
        $o = html_writer::start_tag('li',
            array('class' => 'tcsection main clearfix', 'id' => 'collblct-display-instructions'));
        ...
        $o .= html_writer::tag('p', get_string('instructions', 'format_collblct'),
            array('class' => 'collblct-display-instructions')
        );
        $o .= html_writer::end_tag('div');
        $o .= html_writer::end_tag('div');
        $o .= html_writer::end_tag('li');
